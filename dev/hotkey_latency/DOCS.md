...
`src/menubar/app.py`, `hotkey_controller.py`, `discover.py`, `system.py` write to `menubar.log`
into a distribution report. Does NOT modify `src/` behavior — measurement only.
...
### analyze_latency.py (156 LOC)

Parses `menubar.log`'s `[latency]` lines (default path: `menubar.menubar_log.MENUBAR_LOG`,
override via `argv[1]`) into three buckets — tick phase breakdowns, hotkey queue-delays, focus
lookup/osascript splits — and writes a distribution report (mean/median/p90/p95/max per phase,
slowest N ticks with full breakdown, per-hotkey percentiles) to `md/latency_report_<UTC-ts>.md`.
Imports real menubar code via the `sys.path.insert(0, WORKTREE_ROOT / 'src')` + `from menubar....`
pattern (see `dev/proxy_instrumentation/p1_measure_full_replacement_blast_radius.py` for the
established precedent) — NOT `from src.menubar...` (blocked by the same hook).
