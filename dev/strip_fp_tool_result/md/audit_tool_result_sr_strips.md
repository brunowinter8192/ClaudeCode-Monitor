...
|---|---|---|---|---|---|
| `api_requests_opus_monitor_cc_1785259250_original.jsonl` | 42,404,513 B | 130 | 118 | 11 | 0.3s |
| `api_requests_opus_posts_1785266871_original.jsonl` | 36,625,867 B | 141 | 125 | 12 | 0.2s |
| `api_requests_opus_wise2627_1785240377_original.jsonl` | 2,322,127,935 B | 361 | 354 | 4 | 3.2s |
...
|---|---|---|
| `api_requests_opus_monitor_cc_1785259250_original.jsonl` | 33 | 0 |
...
29 unique occurrences across 5 scanned files. Offset/context are taken from `_block_inner_text(block)` — for `tool_result_list_joined` that is the sub-blocks' text joined with `\n`, NOT any single sub-block's own text.
...
- **Raw occurrences (dedup collapsed):** 230
...
- **Raw occurrences (dedup collapsed):** 97
...
- **Raw occurrences (dedup collapsed):** 92
...
- **Raw occurrences (dedup collapsed):** 72
...
- **Raw occurrences (dedup collapsed):** 68
...
- **Raw occurrences (dedup collapsed):** 4
...
### Occurrence 12: `stripped_hook_error_prefix` via `_apply_hook_prefix_strip`
...
- **Raw occurrences (dedup collapsed):** 160
...
- **Raw occurrences (dedup collapsed):** 101
...
- **Raw occurrences (dedup collapsed):** 96
...
- **Raw occurrences (dedup collapsed):** 91
...
- **Raw occurrences (dedup collapsed):** 73
...
### Occurrence 22: `stripped_hook_error_prefix` via `_apply_hook_prefix_strip`
...
- **Raw occurrences (dedup collapsed):** 12
...
### Occurrence 24: `stripped_po_preview` via `_apply_po_preview_strip`
...
- **Raw occurrences (dedup collapsed):** 354
...
### Occurrence 25: `stripped_hook_error_prefix` via `_apply_hook_prefix_strip`
...
- **Raw occurrences (dedup collapsed):** 284
...
### Occurrence 26: `stripped_po_preview` via `_apply_po_preview_strip`
...
- **Raw occurrences (dedup collapsed):** 70
...
### Occurrence 29: `stripped_bg_launch_ack` via `_apply_bg_launch_ack_strip`
...
| Verdict | Count |
|---|---|
| genuine CC injection | 28 |
| quoted data | 1 |

## Genuine CC injection inside tool_result — found?

**YES — 28 occurrence(s) classified as genuine.** See rows above; the next milestone's fix must NOT blanket-disable tool_result descent.
