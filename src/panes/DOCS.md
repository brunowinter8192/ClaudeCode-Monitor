...
**Writes:** stdout (ANSI screen); `/tmp/monitor_cc_error.log` on caught exception (via `pane_error_log`); mutates module-level `cache_expand_states`, `cache_line_map`, `cache_hover_row`, `cache_scroll_offset`, `_cache_turns`, `_cache_jsonl_position`, `_response_log_pos`, `_response_rid_map`.
...
**Writes:** stdout (ANSI screen); `/tmp/monitor_cc_error.log` on caught exception (via `pane_error_log`); rebinds `error_line_map` from render return; extends `tool_errors`.
