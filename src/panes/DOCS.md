...
- Zebra/hover/truncation render loop lives in `_build_tokens_output()` in `token_pane.py`, NOT in `token_format.py`. `format_cache_tracker` returns a uniform 5-tuple `(visible_lines, visible_keys, sticky_header, viewport_start, initial_parent_count)` on all paths including empty turns (fixed 2026-05-12, commit `1f887ae`) — this shape is unchanged even after the 2026-08-18 search-highlight params (all optional, `nav_out` is an out-param not a return value).
