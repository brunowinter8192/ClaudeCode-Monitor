...
**Calls out:** `discovery`, `render`, `search`, `timeline`, `overlay`, `usage` (all package-local; `overlay.build_overlay` from both `_run_expand` and `_run_msgs`; `overlay.build_sys_tool_overlay` only from `_run_msgs`; `usage.build_usage_by_flow` from `_run_msgs` always and from `_run_reqs` when `--rebuild`/`--drop` is set; `timeline.resolve_req_range` only from `_run_msgs`'s `--req` path; `discovery.filter_by_family` only from `_run_reqs`).
...
**`--drop`'s "previous request" is POSITIONAL in the chronological sequence, not the previous REQ
NUMBER.** REQ numbers restart at 1 per session (see the `--merged` Gotcha above), so under
`--merged` a session's own REQ 1 is not automatically exempt from `--drop` — only the entry
literally FIRST in the merged, globally-sorted chain (`entries[0]`) has no predecessor. A later
session's REQ 1, landing anywhere else in that chain, is compared against whichever OTHER session's
request happens to sit immediately before it in time. Without `--merged`, "positional" and
"per-session REQ 1" coincide, so this distinction is invisible — it only matters once entries from
different sessions interleave.
