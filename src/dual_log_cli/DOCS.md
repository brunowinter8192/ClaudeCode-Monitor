...
**Called by:** `__main__.py`.
**Calls out:** `timeline` (`request_markers`, for `msgs`' REQ separators AND, since 2026-09-04, `reqs`' own listing; `_system_block_chars`/`_tool_chars`, since 2026-09-04, for a sys/tool line's original-chars lookup; `_BILLING_HEADER_SYS_INDEX`, same date, to exempt system index 0 from that lookup).
