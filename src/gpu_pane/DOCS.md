...
**Calls out:** `click_handler` (keyboard via `read_keypress`, mouse via `enable_mouse`/`disable_mouse`/`read_mouse_event`), `status`, `errors`, `pane_error_log` (`log_pane_error`), `utils` (`format_timestamp`, `compute_header_rule_len`), `subprocess.Popen` (rag-cli toggle).
