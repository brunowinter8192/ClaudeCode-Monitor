...
**Writes:** stdout (full-screen ANSI via `\033[2J\033[3J\033[H`); `/tmp/monitor_cc_error.log` on caught exception (via `pane_error_log`); mutates `_gpu_search` (query/focused/matches/match_set/current_idx/drag-select fields).
**Called by:** `workflow.py` (`--mode gpu` route).
**Calls out:** `click_handler` (keyboard via `read_keypress`, mouse via `enable_mouse`/`disable_mouse`/`read_mouse_event`, `copy_to_clipboard`), `status`, `errors`, `pane_error_log` (`log_pane_error`), `utils` (`format_timestamp`, `compute_header_rule_len`, `highlight_query_in_line`), `search_bar` (shared search-bar mechanics), `subprocess.Popen` (rag-cli toggle).
...
| `pane.py` | `_gpu_search: search_bar.SearchState` (2026-08-18) | `.matches` holds 0-based indices into `_render_pane`'s own lines list (no click-interactivity concept for matches -- only buttons are clickable, so no coupling to physical row numbers) | mutated by the inline mouse/key dispatch in `run_gpu_loop` |
...
**`_button_regions` value convention:** `(action, name)` for preset rows; `('stop', 'port-{N}')` for arbitrary rows; `('refresh', 'refresh')` for the header `[refresh]` button (2026-07-30) — the sentinel `action == 'refresh'` is checked FIRST in the mouse dispatch loop so it never reaches `_fire_button`.
