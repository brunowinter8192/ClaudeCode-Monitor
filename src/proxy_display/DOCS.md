...
On expand-click: `forwarded_parser._lazy_load_messages_forwarded(entry, fwd_path)` replays forwarded delta stream from byte 0 to `entry['_fwd_req_idx']`, reconstructs and populates `entry['messages']`. Also reloads `prev_same` in the same click handler.
...
**Called by:** `src/proxy_display/pane.py`, `src/proxy_display/worker_proxy_pane.py`, `src/proxy_display/render_turn.py` (imports `_is_standalone_entry`)
**Calls out:** `format` (token_format)
New private helpers (same module): `_assign_turns_to_entries`, `_apply_row_backgrounds`.

---

### forwarded_parser.py (287 LOC)
...
New private helpers (same module): `_compute_req_mods_str`, `_build_req_header_line`, `_render_req_expanded`.
...
New private helpers (same module): `_render_stripped_block`, `_render_span_content`, `_lookup_spans`, `_render_block_spans`, `_render_new_messages`, `_render_modified_messages`.
