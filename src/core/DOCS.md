...
- **Pre-existing, unfixed by the 2026-08-18 search-bar migration:** `_refresh_main_data` clears `main_event_buffer` on a session change but never resets `_main_search` — a stale `.matches` list of event_idx values can point past the new (shorter) buffer until the next Enter recomputes it. The proxy pane's equivalent (`_refresh_proxy_data`) DOES call `search_bar.handle_search_cancel` on session change; the main pane was left as-is (out of this milestone's scope, not a regression it introduced — the old flat-globals version had the same gap).
