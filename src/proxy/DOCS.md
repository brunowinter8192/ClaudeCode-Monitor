...
**Reads:** `stripped_msg_removed` dict passed by `addon.py` (per-request removed chunks from all message passes — scanned for one whose text is a genuine ack); `worker_context` / `project_path` strings passed by `addon.py` (`self._worker_context`, `PROXY_PROJECT_PATH`).
**Writes:** `_escaped_task_ids` (module-global `set`, in-memory only — see State). Side effect: one `tmux send-keys -t <session> Escape` subprocess call per newly-seen task id.
...
**Called by:** `src/proxy/rules.py` (all 13 functions imported; 12 via `_passes` loop + `_dedup_wakeup_blocks` after)
**Calls out:** `strip_sr`, `content_strip` (`_message_has_rejection`, `_strip_rejection_message`), `payload_helpers`, `rules_config`, `strip_po`, `strip_bg_completed`, `strip_bg_launch_ack`, `strip_hook_prefix`, `strip_git_lock`, `strip_bd_noise`, `strip_sn_notice`, `rule_ops`.
...
- Constants: `BUCKETS` (EFF/INERT/IDX/LEAK/SUS), `RULES` (CMD/SK/AT/DEF/NAG/TN/PYR/UI/PM/REJ/ALL/SC/IR/PP/BGK/BL/GL/BD/ENV/HP/SN/SNP/FM/RS with markers), `TAG_LITERALS` (PO/SR/TN/ND), `STRIP_RULE_CODES`, `_SR_STRIP_RULES` (SR-class strip rule full names, used for LEAK:<SR> detection; excludes TN, SC, IR, PP, SNP).
