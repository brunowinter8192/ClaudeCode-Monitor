...
**Called by:** `src/proxy/message_passes.py` (imports `_ops_from_content_change`); `src/proxy/rules.py` (imports `_merge_ops`)
**Calls out:** —

---

### message_passes.py (547 LOC)

**Purpose:** All fourteen message-level passes delegated by `apply_modification_rules`. Each of the 13 main passes receives `messages: list` and returns a 6-tuple `(new_messages, pass_mods, pass_removed_by_idx, changed_indices, pass_injected_by_idx, pass_ops_by_msg_blk)`. Pass sequence (call order in `rules._passes` loop): `_apply_role_system_strip` (replaces entire content of every `role='system'` message with `"."`, mod: `stripped_role_system_msg` — records its op with `full_replace=True` (2026-07-29 milestone-3, `rule_ops.py`): content is set to the literal `"."` unconditionally, an unambiguous whole-content replacement, so the op is one contiguous `(0, before, ".")` with no prefix/suffix trim; CC 2.1.176 delivers deferred-tools / agent-types / skills as a role=system plain-string message on Opus; idempotency guard skips empty + already-`"."` content; **preserve-guard (CC 2.1.205+):** a `role='system'` message whose `content` is a `str` starting with `_TRUNCATION_NOTICE_MARKER = "[Truncated:"` is appended unchanged instead of nuked — this is CC's Read-partial-view notice (`[Truncated: PARTIAL view — <path>: showing lines X-Y of Z total …]`), and stripping it would leave the agent unaware a Read was incomplete. `startswith`, not substring — anchored to avoid false-positiving on content that merely quotes the marker. Delivered exclusively as a standalone string content role=system message, never embedded in tool_result, per dual-log analysis; **TN preserve-guard (2026-07-29):** a `role='system'` message whose content top-level-contains `<task-notification>` is also appended unchanged instead of nuked — CC delivers bg-task wake-ups as a plain-str role='system' message on some paths (measured: 173/280 real TN occurrences in one session log were role='system'/str vs 107 role='user'/list-text); left untouched here so `_apply_sn_notice_strip` + `_apply_first_pass`'s TN branch (both widened to accept role='system', narrowly gated on the TN tag) build the wake-up exactly once — no duplicated TN-construction logic); `_apply_sn_notice_strip` (strips the bare 4-line `[SYSTEM NOTIFICATION - NOT USER INPUT]` paragraph CC injects ahead of `<task-notification>` tags in bg-task wake-ups — anchored `lstrip().startswith()` decision on top-level str/text-block content only, does NOT descend into tool_result; runs BEFORE `_apply_first_pass` so the TN branch's tag-contains guard and tag-replacement operate on an already-cleaned prefix, mod: `stripped_sn_notice_paragraph`; role gate is `role == 'user'` OR (`role == 'system'` AND top-level-contains `<task-notification>`) — a role='system' message without a TN tag stays out of scope even when the pass is called in isolation, defensive boundary not just a pipeline-order fact); `_apply_first_pass` (elif-chain: plan-mode SR, task-notification tags + wakeup injection, task-tools-nag SR, deferred-tools SR, user-interrupt SR, rejection message — TN branch: both completed and failed call `_replace_task_notification_tags` inline, identical output logic, mod-name only differs: `trimmed_task_notification` for completed, `replaced_task_notification` for failed; **injected_text is up to 3 lines (2026-07-29):** wake-up line, then `Output: <path>` (only if `_extract_task_notification_output_file` returns non-empty), then `ID: <task-id>` (only if `_extract_task_notification_task_id` returns non-empty) — built as a `lines` list joined with `\n` + trailing `\n`, so the neither-present case collapses back to exactly `_WAKEUP_TEXT` (no special-casing needed); order is fixed regardless of which optional lines are present; **TN branch role gate accepts `role in ('user', 'system')`** (2026-07-29, same TN wake-up fix) — every other elif branch (nag/deferred/user-interrupt/rejection) stays `role == 'user'`-only, no measured role='system' occurrence for those; **the task-tools-nag/deferred-tools/user-interrupt branches gate via `_top_level_content_contains`, not `_content_contains`** — 2026-07-28 FP-nuke fix, defense-in-depth alongside `strip_sr.py`'s own tool_result non-descent, same pattern as `_apply_sn_notice_strip`/`_apply_bg_exit_strip` below; the rejection branch is unrelated — `_message_has_rejection` genuinely only ever checks inside tool_result, own marker + length guard, not part of the SR family; **records its op with `full_replace=True`** (2026-07-29 milestone-3) despite `_strip_rejection_message` (`content_strip.py`) mutating PER-BLOCK for list content rather than the whole message at once — safe specifically because every block it actually changes is wholesale-set to the literal `"."` (`content_strip.py:43`), never partially edited, and every block it leaves alone is appended by identity (`content_strip.py:45`) so `bt==at` there and `_extract_block_op`'s `before==after` check returns `[]` before `full_replace` is even consulted; the str branch (`content_strip.py:31`) is unambiguously full-replace too); `_apply_cumulative_sr_strips` (skills-SR, agent-types-SR, claudeMd-SR, pyright-diagnostics — cumulative: applied to every user msg including those already touched by pass 1, diff-based SR extraction; `stripped_agent_types_sr` added for Sonnet-worker standalone agent-types SR ~2,353 chars; **all 4 gates use `_top_level_content_contains`**, same 2026-07-28 fix); `_apply_final_sr_pass` (strips all remaining `<system-reminder>` blocks from top-level content only — no gate of its own, so `strip_sr.py`'s tool_result non-descent is the only thing protecting tool_result content here); `_apply_po_preview_strip` (strips `Preview (first NKB):` section from `<persisted-output>` blocks); `_apply_bg_exit_strip` (replaces first BGK kill notification with `_WAKEUP_TEXT` — id-less, path-less, unchanged by the 2026-07-29 ID-line milestone by design, mod: `replaced_bg_completed_text`); `_apply_bg_launch_ack_strip` (replaces block content with a 3-line hold message — see `strip_bg_launch_ack.py` — only for blocks whose text `lstrip().startswith()` one of TWO anchored prefixes (2026-07-29 milestone-2: wording 1 `'Command running in background with ID:'`, wording 2 `'Command was manually backgrounded by user with ID:'`) — anchored, NOT substring-anywhere; blocks merely containing either phrase mid-content are preserved (FP-nuke fix); fast-path gate OR's the two corresponding markers; all 4 content shapes, mod: `stripped_bg_launch_ack`; **records its op with `full_replace=True`** (2026-07-29 milestone-3) — `_strip_bg_launch_ack` sets a matched block's whole text/content field wholesale, and every block it leaves alone is appended by identity, so the same `bt==at` early-return safety applies; this is the flagship case the milestone was measured against — the shared `"Command "` prefix between the ack and its replacement no longer gets trimmed into its own unhighlighted line); `_apply_hook_prefix_strip` (strips `PreToolUse:<Tool> hook error: [python3 <path>]:` prefix from tool_result content, mod: `stripped_hook_error_prefix`); `_apply_git_lock_strip` (strips constant 5-line git index.lock advice from tool_result content, mod: `stripped_git_lock_advice`); `_apply_bd_noise_strip` (strips bd informational auto-import/export lines from tool_result content, mod: `stripped_bd_noise`); `_apply_interrupt_marker_strip` (2026-07-30 — replaces block content with `'.'` for any block whose text IS EXACTLY `"[Request interrupted by user]"`, CC's rendering of `bg_escape.py`'s tmux-Escape into a worker's pane, never a genuine user interrupt; exact-equality match, not substring-anywhere — see `strip_interrupt_marker.py`; role='user' only; `_content_contains` gate covers all 4 content shapes; mod: `stripped_interrupt_marker`; records its op with `full_replace=True`, same rationale as `_apply_bg_launch_ack_strip`). `_dedup_wakeup_blocks` returns a 2-tuple `(new_messages, ops_by_msg_blk)` — deduplicates multiple `_WAKEUP_TEXT` injections within the same user message to one; called after the 12-pass loop by `rules.apply_modification_rules`. Inject points across passes: (1) TN branch (both failed + completed, role='user' or role='system') → `[_WAKEUP_TEXT]` or the up-to-3-line variant described above; (2) plan-mode FULL-strip branch → hardcoded `["(plan-mode reminder stripped by proxy)"]`; (3) `_apply_bg_exit_strip` when `bg_removed` non-empty → `[_WAKEUP_TEXT]` (id-less/path-less, unaffected by (1)'s ID-line addition). All passes call `_ops_from_content_change` from `rule_ops`.
...
**Called by:** `src/proxy/rules.py` (all 14 functions imported; 13 via `_passes` loop + `_dedup_wakeup_blocks` after)
**Calls out:** `strip_sr`, `content_strip` (`_message_has_rejection`, `_strip_rejection_message`), `payload_helpers`, `rules_config`, `strip_po`, `strip_bg_completed`, `strip_bg_launch_ack`, `strip_hook_prefix`, `strip_git_lock`, `strip_bd_noise`, `strip_sn_notice`, `strip_interrupt_marker`, `rule_ops`.
...
### strip_interrupt_marker.py (79 LOC)

**Purpose:** Replace block content with `'.'` for any block whose text IS EXACTLY `"[Request interrupted by user]"` — CC's rendering of `bg_escape.py`'s tmux-Escape sent into a worker's pane on a backgrounded call, indistinguishable in CC's own log from a genuine user Ctrl-C/Escape interrupt, though no user ever pressed anything. A worker reading the marker halts and waits for an instruction nobody intended to give (measured live: worker replied "I was interrupted right at the point of pivoting to Step 2 … Waiting for the orchestrator's next instruction."). **Exact-equality match** (`text == _INTERRUPT_MARKER`), NOT substring-anywhere — measured (full dual-log corpus scan, 2026-07-30): 1791 occurrences across 4 log files, EVERY occurrence `role='user'`, block `type='text'`, text EXACTLY this string, block index 1 of 3 (preceded by a `tool_result` or `text` block, followed by the proxy's injected wake-up text), never embedded inside longer text — the anchor exists so a genuine user paste/transcript that happens to CONTAIN this phrase inside longer text is never nuked (same FP-nuke class as `strip_bg_launch_ack.py` / `strip_sn_notice.py`). **Emptied-block handling:** the marker occupies its own whole block among others in the measured real shape — content is emptied to `'.'` IN PLACE rather than spliced out of the content list. Splicing would shift every subsequent block's index (the wake-up block would move from index 2 to index 1), corrupting `rule_ops._ops_from_content_change`'s index-keyed old/new comparison (it would diff the marker's OLD text against the wake-up's NEW text as if one block were edited, and see the wake-up block itself as a deletion) — emptying in place keeps block count and indices stable, and matches the `'.'` convention every other pass uses for a block reduced to nothing (the API rejects an empty text block). Covers all 4 content shapes (str, list/text, list/tool_result-str, list/tool_result-list), mirroring `strip_bg_launch_ack.py`'s traversal. Returns `(new_content, removed_chunks)` for `stripped_interrupt_marker` mod attribution via the `IM` rule code (`strip_vocab.py`).
**Reads:** Message content (string or list of blocks).
**Writes:** Nothing — returns `(modified_content, list[str])`.
**Called by:** `src/proxy/message_passes.py` (`_apply_interrupt_marker_strip`).
**Calls out:** —
...
**Reads:** Message content (string or list of blocks).
**Writes:** Nothing — returns `(modified_content, list[str])`.
**Called by:** `src/proxy/message_passes.py` (`_apply_sn_notice_strip`).
**Calls out:** stdlib only.
