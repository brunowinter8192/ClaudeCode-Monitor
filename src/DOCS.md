...
| `monitor_janitor.py` | 70 | `sweep_workflow()` — kills every `monitor_cc_*` tmux session older than 24h (registry-free `tmux list-sessions` enumeration, same lesson as the worker-cli janitor), logs one line per session (name, age, KILLED/SPARED) to `src/logs/monitor_sweep.log`. Invoked as `python3 -m src.monitor_janitor` from project root — from `claude_proxy_start.sh` (detached, every main-session start) and from the `com.brunowinter.monitor-cc-sweep` LaunchAgent (daily, `StartCalendarInterval`, independent of any session — see `setup_monitor_sweep.py`). Never uses `pkill -f` (see `process-docs/pipeline/` `pkill -f` incident) — kills by exact tmux session name only, which tears down all nine panes (verified in `dev/monitor_lifecycle/`). |
| `com.brunowinter.monitor-cc-sweep.plist` | 44 | LaunchAgent template for the daily sweep — `<PROJECT_ROOT>` token substituted by `setup_monitor_sweep.py`; models `menubar`'s plist for the Homebrew-less launchd `PATH` (`tmux` lives in `/opt/homebrew/bin`) |
| `setup_monitor_sweep.py` | 30 | `write_plist()` writes the sweep LaunchAgent to `~/Library/LaunchAgents/`; `bootstrap_command()` returns the `launchctl bootstrap` string to print — never runs it (installing/loading the LaunchAgent is an explicit user step, unlike the menubar's own self-installing `setup_menubar.py`) |
