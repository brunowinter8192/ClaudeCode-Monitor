...
| `monitor_janitor.py` | 87 | `sweep_workflow()` — kills every `monitor_cc_*` tmux session older than 24h (registry-free `tmux list-sessions` enumeration, same lesson as the worker-cli janitor), logs one line per session (name, age, KILLED/SPARED) to `<checkout>/src/logs/monitor_sweep.log`, where `<checkout>` is `$MONITOR_CC_ROOT` if set, else the directory two levels above `monitor_janitor.py`'s own `__file__` — so a manual run from a `.claude/worktrees/<name>/` checkout writes into THAT worktree's `src/logs/`, not the main checkout's, unless `MONITOR_CC_ROOT` is set (both LaunchAgent plists set it explicitly; the bash trigger doesn't need to, since `cd "$MONITOR_CC_ROOT"` before `-m` already makes `__file__` resolve to the right tree). No main-checkout fallback like `dual_log_cli.discovery.resolve_dual_log_dir` — this path is a write target that must follow whichever checkout's code produced the entry, not a read source to prefer aggregating in one place. Invoked as `python3 -m src.monitor_janitor` from project root — from `claude_proxy_start.sh` (detached, every main-session start) and from the `com.brunowinter.monitor-cc-sweep` LaunchAgent (daily, `StartCalendarInterval`, independent of any session — see `setup_monitor_sweep.py`). Never uses `pkill -f` (see `process-docs/pipeline/` `pkill -f` incident) — kills by exact tmux session name only, which tears down all nine panes (verified in `dev/monitor_lifecycle/`). |
| `com.brunowinter.monitor-cc-sweep.plist` | 67 | LaunchAgent template for the daily sweep — `<PROJECT_ROOT>` token substituted by `setup_monitor_sweep.py`; models `menubar`'s plist for the Homebrew-less launchd `PATH` (`tmux` lives in `/opt/homebrew/bin`); also sets `PYTHONPATH`/`MONITOR_CC_ROOT` to `<PROJECT_ROOT>` — `WorkingDirectory` alone does not make `-m src.monitor_janitor` importable under launchd (confirmed live, 2026-09-03). **Requires Full Disk Access for `/usr/bin/python3`** if the checkout lives under `~/Documents`/`~/Desktop`/`~/Downloads` — macOS TCC blocks a plain launchd-spawned process (no Terminal/shell ancestor to inherit an approval from) from even LISTING that directory, which surfaces as the same `ModuleNotFoundError: No module named src` regardless of how correct `PYTHONPATH`/`WorkingDirectory` are (see `process-docs/monitor_lifecycle/`). |
| `setup_monitor_sweep.py` | 44 | `write_plist()` writes the sweep LaunchAgent to `~/Library/LaunchAgents/`; `bootstrap_command()` returns the `launchctl bootstrap` string to print — never runs it (installing/loading the LaunchAgent is an explicit user step, unlike the menubar's own self-installing `setup_menubar.py`); `full_disk_access_note()` reminds the user about the Full Disk Access requirement above |
