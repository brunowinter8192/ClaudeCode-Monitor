...
**Writes:** bar icon; `SETTINGS_FILE` on toggle/resize; `src/logs/menubar.log` ([abort] category via `_abort_bg_sleep_timers` in `bg_timer.py` on manual abort only; [tick] category when `MENUBAR_DIAGNOSTICS=1`).
...
**Called by:** `hotkey_controller.py` (`log_menubar`); `app.py` (`log_menubar`, lazy `cleanup_old_lines` via import inside `_tick`); `bg_timer.py` (`log_menubar` — sole `[abort]` category writer since 2026-08's auto-abort removal); `panel.py` (`log_menubar`); `desktop_detection.py` (`log_menubar`).
...
**Called by:** `discover.py:list_alive_sessions` (refresh calls, incl. `_refresh_bg_task_cache`); `discover.py:_process_project_dir` (query calls + `_tmux_window_activity`); `ghostty.py:_tty_for_cwd` (`_cc_proc_cache` import); `bg_timer.py:_scan_bg_sleep_timers` (`_cc_proc_cache` import); `bg_timer.py:_abort_bg_sleep_timers` (`_TASKS_BASE` import; its own 0-byte check for which wake-up-process output files to mark `'aborted\n'` is a separate, narrower concern than `_has_active_bg` — out of this predicate's scope).
...
- **Abort leaves task file at 0 bytes (verified for the legacy sleep-timer case; unverified for `worker-cli wait`):** for the legacy `sleep N && echo done` timer, killing the sleep child via SIGTERM makes the zsh parent exit via `&&` short-circuit (no `echo done` stdout), so CC writes nothing to the task file — it stays 0 bytes indefinitely. `_abort_bg_sleep_timers` (bg_timer.py) explicitly writes `aborted\n` to all 0-byte task files after kill so `_has_active_bg` returns False and the `[B]` badge disappears. **Not independently verified from this worktree** what CC itself writes to a `worker-cli wait` process's own task file on SIGTERM (would require a live spawn+kill+observe cycle against a real CC session) — the defensive global 0-byte sweep covers this case too regardless of what CC does, since it only ever ADDS the `aborted\n` marker to files that are still empty, never overwrites a file CC already wrote to.
...
- **Abort task-file write is global** (`bg_timer.py:_abort_bg_sleep_timers`): after killing PIDs, writes `aborted\n` to ALL 0-byte `*.output` task files under `_TASKS_BASE`, not just those belonging to the aborted project. Manual abort only now (auto-abort removed 2026-08). If project B has live 0-byte task files when project A is manually aborted, B's `[B]` badge may transiently disappear. In practice rare (requires simultaneous multi-project bg tasks).
