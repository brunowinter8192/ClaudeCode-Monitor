...
**Purpose:** `CCMenuBarApp` (rumps.App subclass) + `_PanelController` (NSObject target for all button actions + NSTextField delegate) + `_tick` timer + blink + bar-icon + settings load/save. Three-panel lifecycle and cycling moved to `panel_lifecycle.py`. Settings load/save moved to `app_settings.py`. Queue action handlers (`addQueueRow_`, `toggleQueueEntry_`, `removeQueueEntry_`, `commitQueueField_`, `controlTextDidEndEditing_`) are 1-line delegates to `self._app.queue.handle_*` methods. `_tick` delegates session snapshot to `self.sessions.refresh()`, auto-focus to `self.focus.tick(sessions, now)` (2026-08: no longer takes `bg_by_project` — auto-abort removed), queue refresh + conditional rebuild to `self.queue.tick(sessions)`, panel rebuild/update to `self.panel.rebuild()` / `self.panel.update_inplace()`, and digit-hotkey re-registration to `self.hotkey.reregister_digits(self.panel._desktop_to_cwd)`; status snapshot updated via `self.focus.update_statuses(sessions)` at tick-end. `_PanelController.abortBgTimer_` (manual abort, panel button) is the sole abort trigger left — unaffected by the auto-abort removal. **Latency instrumentation (2026-08, measurement only — see `process-docs/hotkey_latency/`):** `_tick` times every sub-phase (`time.monotonic()` before/after each) into a local `phases` dict — `sessions_refresh_total` + the 6 sub-phases merged in via `discover.py:get_last_session_timings()`, plus `bg_timer_scan`, `focus_tick`, `queue_tick`, `rag_tick`, `panel_rebuild_update`. When total tick duration exceeds `TICK_LATENCY_THRESHOLD_MS` (module constant, default 200ms) logs ONE `[latency]` line with the full breakdown; below threshold, no log (near-zero cost: monotonic() calls + dict writes only).
**Reads:** `self.sessions.refresh()` (via `SessionsController`) + `_scan_bg_sleep_timers()` on every tick and on panel open; `SETTINGS_FILE` on launch; `discover.py:get_last_session_timings()` (per-tick, for the latency breakdown).
...
**Called by:** `app.py:CCMenuBarApp._tick`, `app.py:_open_main_panel`, `app.py:_PanelController.*Queue*` methods; `get_last_session_timings()` called by `app.py:CCMenuBarApp._tick` only.
...
### proc_cache.py (184 LOC, orchestrator-signal reader removed 2026-08)
...
**Writes:** module-level caches (`_cc_proc_cache`, `_tmux_state_cache`, `_bg_task_open_paths`, `_proxy_log_mtime_cache`, `_hook_state_cache`).
**Called by:** `discover.py:list_alive_sessions` (refresh calls, incl. `_refresh_bg_task_cache`); `discover.py:_process_project_dir` (query calls + `_tmux_window_activity`); `ghostty.py:_tty_for_cwd` (`_cc_proc_cache` import); `bg_timer.py:_scan_bg_sleep_timers` (`_cc_proc_cache` import). `bg_timer.py:_abort_bg_sleep_timers` no longer imports from this module (2026-08-18: dropped the `_TASKS_BASE` import — resolves each killed PID's own output file via a direct `lsof -p <pid>` call instead of scanning `_TASKS_BASE` globally).
...
**Called by:** `app.py:CCMenuBarApp._tick` (`_scan_bg_sleep_timers`); `app.py:_PanelController.abortBgTimer_` (`_scan_bg_sleep_timers`, `_abort_bg_sleep_timers` — manual abort, panel button, UNAFFECTED by the 2026-08 auto-abort removal in `focus_controller.py`); `app.py:_PanelController.windowDidEndLiveResize_` (`_scan_bg_sleep_timers`).
...
- **`_refresh_ghostty_tty_to_id`'s `_GHOSTTY_TTY_REFRESH_INTERVAL=10.0` does NOT gate the steady-state case** (measured via `[latency]` instrumentation, live production bundle, 2026-08-19 — see `process-docs/hotkey_latency/`): `_ghostty_tty_last_refresh` is only updated when `new_ttys` is non-empty (step 10 above); once every current Ghostty TTY is already mapped (the common steady state), the function returns early WITHOUT updating the timestamp, so the `now - _ghostty_tty_last_refresh < 10.0` guard never re-arms — `_ghostty_pid()` + `_ghostty_child_ttys()` (2 `ps -A` calls) run on EVERY tick, not once per 10s. Measured cost: ~150-165ms of the `ghostty` tick-phase on nearly every 1.5s tick in the live bundle — the single largest recurring contributor keeping steady-state ticks just over `TICK_LATENCY_THRESHOLD_MS=200`. Not fixed as of this measurement (out of scope for the measurement-only milestone) — candidate for a future fix: track "TTY set unchanged" separately from "new TTY probed".
